<script>
    ...
    let statsArray;
    export let missingPoetCount = -1;
    export let select;

    let svg1;
    let xScale, yScale;

    $: if (rankData && missingPoetCount) {
        let countStats = {};
    ...
    function drawChart() {
        ...
        chart
            ...
            .attr("fill", (d) => d.unknownCount === missingPoetCount || missingPoetCount === -1 ? 'steelblue' : 'grey')
            .on("click", (event, d) => {
                if (select) {
...
<div bind:clientWidth={width} bind:clientHeight={height} style="height: 15%">
    <svg bind:this={svg1} width="100%" height="100%" id="svg1"></svg>
</div>